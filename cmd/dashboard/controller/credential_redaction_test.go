...
// GHSA 同类信息泄露：GET /api/v1/profile 曾把用户密码哈希（bcrypt）随 User 对象
// 明文回显。User.Password 现为 json:"-" 永不序列化；密码输入走 ProfileForm，不受影响。
...
// GHSA-ww5p-j6cj-6mqq：GET /api/v1/ddns 与 /api/v1/notification 列表接口曾明文返回
// 第三方凭据（Cloudflare/腾讯云密钥、含 bot token 的 webhook URL、Authorization 头）。
// 列表端点现脱敏这些字段；因前端编辑表单从列表回填，update 处理器把空值视为"不修改"
// 以保留旧值，避免脱敏导致误清空。
...
func TestListDDNS_RedactsCredentials(t *testing.T) {
	...
	c := ctxAsUser(10, model.RoleAdmin)
	out, err := listDDNS(c)
	require.NoError(t, err)
	require.Len(t, out, 1)
