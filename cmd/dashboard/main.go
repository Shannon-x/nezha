...
type DashboardCliParam struct {
	...
	DatabaseLocation string // Sqlite3 数据库文件路径 (向后兼容，推荐使用配置文件)
}
...
// 优先使用配置文件中的数据库配置，如果未配置则使用命令行参数 (向后兼容)
// Initialize database connection. Uses config file settings with CLI override for backward compatibility.
func initDatabase() error {
	dbConfig := singleton.Conf.Database

	// 向后兼容: 如果配置文件中未指定数据库类型，且命令行指定了 -db 参数，则使用 SQLite
	// Backward compatibility: If database type is not specified in config and -db CLI flag is provided, use SQLite
	if dbConfig.Type == "" {
		dbConfig.Type = model.DBTypeSQLite
		if dashboardCliParam.DatabaseLocation != "" {
			dbConfig.Path = dashboardCliParam.DatabaseLocation
		}
	}

	return singleton.InitDB(dbConfig)
}
